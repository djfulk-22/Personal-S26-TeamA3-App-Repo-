...
def add_batting_metrics(df, season=True, include_weighted=False):
    """
    ...
    df.loc[:, 'BABIP'] = round((df['H'] - df['HR'])
                               / (df['AB'] - df['K'] - df['HR']
                                  + df['SF']), ROUND_TO)
    if season and include_weighted:
        ...
        # keep columns present but disabled
        df.loc[:, 'wOBA'] = np.nan
        df.loc[:, 'wRAA'] = np.nan
        df.loc[:, 'wRC'] = np.nan

    weighted_cols = ['wOBA', 'wRAA', 'wRC', 'FIP', 'wOBA-against']
    to_fill = [c for c in weighted_cols if c in df.columns]
    df = df.fillna(value=0.00, inplace=False)
    for c in to_fill:
        # if we disabled them, keep them as NaN
...
def add_pitching_metrics(df, season=True, include_weighted=False):
    """
    Adds the following columns to a given DataFrame:
