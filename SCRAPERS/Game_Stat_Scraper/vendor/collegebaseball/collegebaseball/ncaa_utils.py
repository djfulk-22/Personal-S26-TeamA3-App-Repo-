...
# --- Extend headers forward using last known NCAA format
# If NCAA changes columns again, update the base year to the last correct season.
_FORWARD_YEARS = [2024, 2025, 2026]  # add more if you want
_BASE_YEAR = 2023

for v in ["batting", "pitching", "fielding"]:
    base = team_gamelog_headers[v][_BASE_YEAR]
    for yr in _FORWARD_YEARS:
        if yr not in team_gamelog_headers[v]:
            team_gamelog_headers[v][yr] = base


# --- For 2024+ pitching, NCAA appears to have dropped the leading "G" column.
_PITCHING_STATS_2024P = [
    "App", "GS", "IP", "CG", "H", "R", "ER", "BB", "SO", "SHO", "BF",
    "P-OAB", "2B-A", "3B-A", "Bk", "HR-A", "WP", "HB", "IBB",
    "Inh Run", "Inh Run Score", "SHA", "SFA", "Pitches", "GO", "FO",
    "W", "L", "SV", "OrdAppeared", "KL", "pickoffs"
]

_PITCHING_META = [
    "date", "field", "season_id", "opponent_id", "opponent_name",
    "innings_played", "extras", "runs_scored", "runs_allowed",
    "run_difference", "result", "game_id", "school_id"
]

for yr in [2024, 2025, 2026]:
    team_gamelog_headers["pitching"][yr] = _PITCHING_STATS_2024P + _PITCHING_META
